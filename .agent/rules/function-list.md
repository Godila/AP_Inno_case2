...
Returns:
  ...
  Schema: .agent/mcp-functions/search/search_web.json
- `search.search_news` — 
...
Returns:
  ...
  Schema: .agent/mcp-functions/search/search_news.json
...
Returns:
  ...
  Schema: .agent/mcp-functions/search/search_images.json
...
Returns:
  ...
  Schema: .agent/mcp-functions/search/search_videos.json
...
- `my.assessRisk` — Сохраняет риск-профиль объекта и выносит решение по заявке: принять, эскалировать или отказать. Вызывай после каждой реплики агента. Передавай только те параметры, которые прозвучали в этой реплике, остальные не передавай.
  Directory: functions/my/assessRisk/
- `policy.pdfspike` — pdfspike
  Directory: functions/policy/pdfspike/
- `policy.issuePolicy` — Выпускает полис: присваивает номер, формирует бланк PDF и записывает сделку в реестр. Вызывай только после того, как оценка риска дала решение accepted и человек подтвердил выпуск. Параметров не требует, все данные берутся из сессии.
  Directory: functions/policy/issuePolicy/
